...
class AutoMod(commands.Cog):
    ...
    async def anti_invites(self, message: disnake.Message):
        ...
        if matches:
            guild = self.bot.get_guild(1116770122770685982)
            askro_invites = [inv.code for inv in await guild.invites()]
            try:
                askro_invites.append((await guild.vanity_invite()).code)
            except disnake.HTTPException:
                pass
            if any(inv for inv in matches if inv not in askro_invites):
                await utils.try_delete(message)
    ...
    async def on_message(self, message: disnake.Message):
        if message.author.bot or message.author.id not in self.bot.owner_ids or\
                not message.guild or message.guild.id != 1116770122770685982 or \
    ...
    async def on_message_edit(self, before: disnake.Message, after: disnake.Message):
        if after.author.bot or after.author.id not in self.bot.owner_ids or \
                not after.guild or after.guild.id != 1116770122770685982 or \
